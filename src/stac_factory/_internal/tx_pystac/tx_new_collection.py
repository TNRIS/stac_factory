...
from _internal.tx_aws.s_three import Collection as S3Collection
from _internal.util import log_info, log_exception, stream_handler
...
class TxNewCollection(TxCollection):
    ...
    def build_metadata_from_input(self):
        ...
        self.extra_fields["txgio:categories"] = self.whcollection.get(
            "txgio:categories"
        )
        self.extra_fields["txgio:publication_date"] = self.whcollection.get(
            "publication_date"
        )
        if self.whcollection.get("txgio:banner_text"):
            self.extra_fields["txgio:banner_text"] = self.whcollection.get(
                "txgio:banner_text"
            )
        ...
        if self.whcollection.get("txgio:spatial_keywords"):
            self.extra_fields["txgio:spatial_keywords"] = self.whcollection.get(
                "txgio:spatial_keywords"
            )

        self.extra_fields["txgio:spatial_reference"] = self.whcollection.get(
        ...
        self.extra_fields["txgio:resolution"] = self.whcollection.get(
            "txgio:resolution"
        )
        ...
        self.extra_fields["txgio:last_edited_by"] = (
            self.whcollection.get("txgio:last_edited_by") or "PLACEHOLDER"
        )
        self.extra_fields["txgio:template"] = (
            self.whcollection.get("txgio:template") or "PLACEHOLDER"
        )
