...
class TxAsset(pystac.Asset):
    ...
    """
    def __init__(self, file_type: str, rsc_path: str, title: str) -> dict[str, pystac.Asset]:

        stac_assets: dict[str, pystac.Asset] = {}
        stac_asset = pystac.Asset(href=rsc_path, title=title, description=f"{file_type} file", media_type=f"{file_type}")
