...
class TxItem(pystac.Item):
    ...
    def get_file(self, rsc: Resource):
        try:
            return remotezip.RemoteZip(self.wh_client.get_filename_path(rsc.path)).filelist
        except Exception as e:
            log_info(f"{self.wh_client.get_filename_path(rsc.path)} cannot be unzipped. Proceeding to next item without making a stac resource.")
            return None
    ...
    def build_dem_stac(self, rsc: Resource):
        ...
        for file in zip_files:
            if(file.filename.endswith(".img")):
                ...
                return [ginfo["wgs84Extent"], pystac.utils.geometry_to_bbox(ginfo["wgs84Extent"])] 
