        ...
        if(len(resources)):
            ...
            for r in df.itertuples():
                ...
                if resource_type == "assets": 
                    ...
                    if(len(resource_parts) == 7): # If directory parts is only 7 then this asset is statewide so check.
                        type = "statewide"
...
class BucketClient:
    ...
    def get_dirs(self, prefix: str, delimiter: str | None = None) -> List[ListObjectsV2OutputTypeDef]:
        """
...
class WarehouseClient(BucketClient):
            ...
            for dir in self.get_dirs(collection_name): #self.archive_extension):
                ...
                for path in paths:
                    if(len(path.get("Key").split("/")) >= 8):
                        resources.append(Resource(path=path, collection_name=collection_name))
    ...
    def get_all_data_warehouse_collections(self) -> List[Collection]:
        ...
        if(not s3_collections):
            raise(IndexError(f"There are no resources in the collection: {s3_collections.name}"))
